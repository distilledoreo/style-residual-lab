...
const residualExperiment = {
  ...
  note: "Diagnostic for the residualization experiment; not an operational acceptance gate. Model selection picks the best validation performer regardless of vector space."
};
const report = {
  selectedModel: {
...
function residualVsRawAssessment(testComparison: Awaited<ReturnType<typeof testSplitModelComparison>>) {
  ...
  const bestRawTestModel = [...testComparison.filter((item) => item.id.startsWith("raw"))].sort((a, b) => {
